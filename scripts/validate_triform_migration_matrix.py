...
def main():
    ...
    if dc.get('execution_authority') is not False or dc.get('authority_effect') is not False or dc.get('activation_effect') is not False or dc.get('final_cross_repository_validity') is not False or dc.get('historical_source_replacement') is not False or dc.get('workflow_authority_effect')!='NONE_VALIDATION_ONLY' or dc.get('readme_status_reconciled') is not True: f.append('dc_authority_or_readme_boundary_drift')
    ...
    if data.get('next_executable_candidate')!='Admissible-Existence/Triad': f.append('unexpected_next_executable_candidate')
    if data.get('selection_evidence_state')!='EVIDENCE_PASS_COMPLETE': f.append('selection_evidence_state_mismatch')
    required={'Admissible-Existence/Triad@main:docs/TRIAD_MIRROR_HANDOFF.md','Admissible-Existence/Triad@main:formalism/principle-registry.yaml','Admissible-Existence/Triad@main:README.md','Admissible-Existence/Triad#1'}
    if not required.issubset(set(data.get('selection_evidence',[]))): f.append('triad_selection_evidence_incomplete')
    if triad.get('triform_state')!='SELECTED_NEXT_EXECUTABLE_CANDIDATE': f.append('triad_selection_state_mismatch')
    if triad.get('prior_goal')!='TRIAD-PRINCIPLE-COMPLETENESS-001' or triad.get('prior_goal_state')!='COMPLETE_RELEASED_HOSTED_VALIDATED': f.append('triad_prior_goal_state_mismatch')
    if triad.get('repository_local_archive_readiness') is not True: f.append('triad_archive_readiness_must_be_true')
    if triad.get('historical_stable_ids')!=['TRIAD-SUBJECT-STANDING','TRIAD-BOUNDARY-STANDING','TRIAD-GOVERNANCE-STANDING']: f.append('triad_historical_ids_mismatch')
    if triad.get('principle_count')!=3 or triad.get('proof_maturity')!='tested_not_proven': f.append('triad_principle_or_maturity_mismatch')
    if triad.get('execution_authorized') is not False or triad.get('publication_authorized') is not False or triad.get('proofs_accepted') is not False: f.append('triad_authority_boundary_mismatch')
    if triad.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('triad_final_admissibility_authority_mismatch')
    if triad.get('readme_status_reconciliation_required') is not True: f.append('triad_readme_reconciliation_must_be_required')
    for e in entries:
