...
def main():
    ...
    if data.get('next_executable_candidate')!='Admissible-Existence/IICT': f.append('unexpected_next_executable_candidate')
    if data.get('selection_evidence_state')!='EVIDENCE_PASS_COMPLETE': f.append('selection_evidence_state_mismatch')
    required={'Admissible-Existence/IICT@main:IICT_MIRROR_HANDOFF.md','Admissible-Existence/IICT@main:formalism/principle-registry.yaml','Admissible-Existence/IICT@main:formalism/proof-candidates.yaml','Admissible-Existence/IICT@main:reports/iict-principle-completeness-validation.json','Admissible-Existence/IICT@main:README.md','Admissible-Existence/IICT#1'}
    if not required.issubset(set(data.get('selection_evidence',[]))): f.append('iict_selection_evidence_incomplete')
    if iict.get('triform_state')!='SELECTED_NEXT_EXECUTABLE_CANDIDATE': f.append('iict_selection_state_mismatch')
    ...
    if iict.get('execution_authorized') is not False or iict.get('publication_authorized') is not False or iict.get('proofs_accepted') is not False: f.append('iict_authority_boundary_drift')
    if iict.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('iict_final_admissibility_authority_mismatch')
    if iict.get('readme_status_reconciliation_required') is not True: f.append('iict_readme_reconciliation_must_be_required')
    for e in entries:
