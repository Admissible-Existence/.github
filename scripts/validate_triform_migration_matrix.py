...
def main():
    ...
    if cta.get('triform_state')!='DEFER_ACTIVE_CANONICAL_CLAIM' or cta.get('claim_state')!='ACTIVE_INTEGRATION' or cta.get('active_goal')!='Complete CTA automation and provenance-bound integration': f.append('cta_active_collision_not_preserved')
    if 'issue:1' not in set(cta.get('evidence',[])): f.append('cta_collision_evidence_missing')
    if data.get('next_executable_candidate')!='Admissible-Existence/HPS': f.append('unexpected_next_executable_candidate')
    if data.get('selection_evidence_state')!='EVIDENCE_PASS_COMPLETE': f.append('selection_evidence_state_mismatch')
    required_selection={'Admissible-Existence/HPS@main:HPS_MIRROR_HANDOFF.md','Admissible-Existence/HPS@main:formalism/principle-registry.yaml','Admissible-Existence/HPS@main:formalism/proof-candidates.yaml','Admissible-Existence/HPS@main:reports/hps-principle-completeness-validation.json','Admissible-Existence/HPS@main:README.md','Admissible-Existence/HPS#2'}
    if not required_selection.issubset(set(data.get('selection_evidence',[]))): f.append('hps_selection_evidence_incomplete')
    if hps.get('triform_state')!='SELECTED_NEXT_EXECUTABLE_CANDIDATE': f.append('hps_selection_state_mismatch')
    if hps.get('prior_goal')!='HPS-PRINCIPLE-COMPLETENESS-001' or hps.get('prior_goal_state')!='COMPLETE_VALIDATED_RELEASED': f.append('hps_prior_goal_state_mismatch')
    if hps.get('repository_local_archive_readiness') is not True: f.append('hps_archive_readiness_must_be_true')
    if hps.get('historical_stable_ids')!=['HPS-P-001','HPS-P-002','HPS-P-003','HPS-P-004'] or hps.get('principle_count')!=4: f.append('hps_historical_ids_or_count_mismatch')
    if hps.get('proof_candidate_maturities')!=['tested_not_universal_proof','tested_not_universal_proof','tested_not_universal_proof','tested_non_authority_contract']: f.append('hps_candidate_maturity_drift')
    if hps.get('standing_restoration_requires_current_reconstructable_evidence') is not True or hps.get('capability_standing_is_window_bound') is not True or hps.get('standing_classification_is_fail_closed') is not True: f.append('hps_core_semantic_boundary_drift')
    if hps.get('visualization_grants_authority') is not False or hps.get('execution_authorized') is not False or hps.get('publication_authorized') is not False or hps.get('proofs_accepted') is not False or hps.get('downstream_activation_authorized') is not False: f.append('hps_authority_boundary_drift')
    if hps.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('hps_final_admissibility_authority_mismatch')
    if hps.get('readme_status_reconciliation_required') is not True: f.append('hps_readme_reconciliation_must_be_required')
    for e in entries:
