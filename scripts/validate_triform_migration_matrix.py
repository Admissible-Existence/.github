...
EXPECTED_COMPLETED = [
    ...
    'Admissible-Existence/FI','Admissible-Existence/DaCo'
]
...
def main():
    ...
    require(data.get('refresh_goal_id') == 'AEX-TRIFORM-MIGRATION-REFRESH-016', 'refresh_goal_id_mismatch', f)
    ...
    require(data.get('completed_source_migrations') == 16, 'completed_source_migration_count_mismatch', f)
    ...
    require(data.get('next_executable_candidate') is None, 'next_executable_candidate_must_reset_after_daco_registration', f)
    require(data.get('selection_evidence_state') == 'EVIDENCE_PASS_REQUIRED', 'selection_evidence_state_mismatch', f)
    require({
        'Admissible-Existence/DaCo#2:CLOSED_COMPLETED',
        'Admissible-Existence/DaCo#3:MERGED',
        'Admissible-Existence/DaCo@0f5b70310b94fb7e88dc6785131ddf979727a809:DaCo Validation run 34074584865 job 101598103222 SUCCESS',
        'Admissible-Existence/DaCo@a5e4e2969fad89b612212eb8743ae52a420c0616:MERGED'
    }.issubset(set(data.get('selection_evidence',[]))), 'daco_completion_evidence_incomplete', f)
    require(daco.get('triform_state') == 'BOUNDED_TRIFORM_COMPLETE_MERGED', 'daco_completion_not_recorded', f)
    ...
    require(daco.get('proof_candidate_maturities') == ['bounded_candidate_not_universal_proof','bounded_candidate_not_universal_proof','bounded_candidate_not_universal_proof','boundary_assertion_testable_not_authority'], 'daco_candidate_maturity_drift', f)
    ...
    require(daco.get('master_records_custody') is False and daco.get('historical_source_replacement') is False and daco.get('workflow_authority_effect') == 'NONE_VALIDATION_ONLY' and daco.get('readme_status_reconciled') is True, 'daco_preservation_boundary_drift', f)
    require({'docs/DACO_MIRROR_HANDOFF.md','docs/DACO_TRIFORM_MIRROR_HANDOFF.md','formalism/principle-registry.yaml','formalism/dependency-graph.yaml','formalism/proof-candidates.yaml','formalism/triform-counterpart-inventory.json','formalism/triform-manifest.json','README.md','issue:2','PR:3','merge:a5e4e2969fad89b612212eb8743ae52a420c0616','validation_run:34074584865','validation_job:101598103222'}.issubset(set(daco.get('evidence',[]))), 'daco_source_evidence_incomplete', f)
