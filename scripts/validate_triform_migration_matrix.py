...
def main():
    ...
    if gcat.get('proposal_is_permission') is not False: f.append('gcat_proposal_permission_collapse')
    if gcat.get('unknown_or_contradictory_required_evidence_is_allow') is not False: f.append('gcat_fail_closed_boundary_drift')
    if gcat.get('replay_renews_current_authority') is not False: f.append('gcat_replay_authority_drift')
    ...
    if gcat.get('historical_source_replacement') is not False: f.append('gcat_historical_source_replacement_must_be_false')
    if gcat.get('workflow_authority_effect')!='NONE_VALIDATION_ONLY': f.append('gcat_workflow_authority_effect_mismatch')
    if gcat.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('gcat_final_admissibility_authority_mismatch')
    if gcat.get('readme_status_reconciled') is not True: f.append('gcat_readme_reconciliation_missing')
