...
def main():
    ...
    if data.get('refresh_goal_id')!='AEX-TRIFORM-MIGRATION-REFRESH-012': f.append('refresh_goal_id_mismatch')
    ...
    if ecat.get('execution_authorized') is not False or ecat.get('publication_authorized') is not False or ecat.get('proofs_accepted') is not False or ecat.get('final_cross_repository_validity') is not False: f.append('ecat_authority_boundary_drift')
    if ecat.get('historical_source_replacement') is not False or ecat.get('workflow_authority_effect')!='NONE_VALIDATION_ONLY' or ecat.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('ecat_preservation_boundary_drift')
    if ecat.get('readme_status_reconciled') is not True: f.append('ecat_readme_reconciliation_missing')
    required_ecat_evidence={'docs/ECAT_ICAT_MIRROR_HANDOFF.md','docs/ECAT_ICAT_TRIFORM_MIRROR_HANDOFF.md','formalism/triform-counterpart-inventory.json','formalism/triform-manifest.json','PR:3'}
    if not required_ecat_evidence.issubset(set(ecat.get('evidence',[]))): f.append('ecat_completion_evidence_incomplete')
