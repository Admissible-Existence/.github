...
def main():
    ...
    if triad.get('review_standing_is_execution_authority') is not False: f.append('triad_review_execution_boundary_drift')
    if triad.get('subject_boundary_governance_standing_collapsed') is not False: f.append('triad_standing_distinction_drift')
    if triad.get('prior_review_substitutes_for_commit_time_governance') is not False: f.append('triad_commit_time_governance_drift')
    if triad.get('unknown_required_standing_is_allow') is not False: f.append('triad_fail_closed_unknown_drift')
    if triad.get('execution_authorized') is not False or triad.get('publication_authorized') is not False or triad.get('proofs_accepted') is not False: f.append('triad_authority_boundary_drift')
    if triad.get('historical_source_replacement') is not False: f.append('triad_historical_source_replacement_must_be_false')
    if triad.get('workflow_authority_effect')!='NONE_VALIDATION_ONLY': f.append('triad_workflow_authority_effect_mismatch')
    if triad.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('triad_final_admissibility_authority_mismatch')
    if triad.get('readme_status_reconciled') is not True: f.append('triad_readme_status_must_be_reconciled')
