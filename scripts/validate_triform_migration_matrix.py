...
def main():
    ...
    if data.get('next_executable_candidate')!='Admissible-Existence/ECAT-ICAT': f.append('unexpected_next_executable_candidate')
    if data.get('selection_evidence_state')!='EVIDENCE_PASS_COMPLETE': f.append('selection_evidence_state_mismatch')
    required={'Admissible-Existence/ECAT-ICAT@main:docs/ECAT_ICAT_MIRROR_HANDOFF.md','Admissible-Existence/ECAT-ICAT@main:formalism/principle-registry.yaml','Admissible-Existence/ECAT-ICAT@main:formalism/proof-candidates.yaml','Admissible-Existence/ECAT-ICAT@main:reports/ecat-icat-principle-completeness-validation.json','Admissible-Existence/ECAT-ICAT@main:README.md','Admissible-Existence/ECAT-ICAT#1'}
    if not required.issubset(set(data.get('selection_evidence',[]))): f.append('ecat_selection_evidence_incomplete')
    if ecat.get('triform_state')!='SELECTED_NEXT_EXECUTABLE_CANDIDATE': f.append('ecat_selection_state_mismatch')
    if ecat.get('prior_goal')!='ECAT-ICAT-PRINCIPLE-COMPLETENESS-001' or ecat.get('prior_goal_state')!='COMPLETE_RELEASED_HOSTED_VALIDATED': f.append('ecat_prior_goal_state_mismatch')
    if ecat.get('repository_local_archive_readiness') is not True: f.append('ecat_archive_readiness_must_be_true')
    if ecat.get('historical_stable_ids')!=['ECAT-001','ICAT-001','ECAT-ICAT-001','ECAT-ICAT-002']: f.append('ecat_historical_ids_mismatch')
    if ecat.get('principle_count')!=4: f.append('ecat_principle_count_mismatch')
    if ecat.get('proof_candidate_maturities')!=['tested_candidate','tested_candidate','bounded_candidate']: f.append('ecat_candidate_maturity_mismatch')
    ...
    if ecat.get('execution_authorized') is not False or ecat.get('publication_authorized') is not False or ecat.get('proofs_accepted') is not False: f.append('ecat_authority_boundary_drift')
    if ecat.get('final_admissibility_authority')!='Admissible-Existence/AE': f.append('ecat_final_admissibility_authority_mismatch')
    if ecat.get('readme_status_reconciliation_required') is not True: f.append('ecat_readme_reconciliation_must_be_required')
    for e in entries:
